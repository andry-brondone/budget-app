    ...
    <div className="max-h-[60vh] space-y-2 overflow-y-auto">
      {categories?.map((category) => {
        ...
        return (
          <div
            key={category.id}
            className="flex items-center justify-between rounded-xl border border-slate-100 px-3 py-2 dark:border-ink-700"
          >
            <div className="flex min-w-0 items-center gap-2">
              <Icon size={16} style={{ color }} className="shrink-0" />
              <span className="truncate text-sm text-slate-800 dark:text-slate-200">{category.name}</span>
              <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-ink-700 dark:text-slate-400">
